"""Iter 11 lockbox: pre-register a winning (item, self-norm-cross variant) pair and run LOOCV.

Mirrors run_interaction_lockbox.py but uses run_self_norm_cross.VARIANTS.

Usage:
  python run_self_norm_cross_lockbox.py --item 12 --variant self_norm_cccv2 --tag iter11
"""
...
from run_self_norm_cross import VARIANTS, load_data, _filter_nan, run_one
...
def main() -> None:
    ...
    p.add_argument("--tag", type=str, default="iter11")
    args = p.parse_args()
    ...
    prereg = {
        ...
        "n_subjects": 0,
        "rationale": "Iter 11 self-norm × cccv2 × hy_residual cross-product winner.",
        "git_status": "iter11-self-norm-cross",
    }
    prereg_path = out_dir / f"preregistration_self_norm_cross_{args.item}_{args.variant}_{timestamp}.json"
    ...
    print(f"[lockbox] Running LOOCV: item={args.item} variant={args.variant} N={len(y)} seeds={args.seeds}",
          flush=True)
    t0 = time.time()
    result = run_one(d, args.item, args.variant, "loocv",
                    seeds=tuple(args.seeds), with_null=False)
    elapsed = time.time() - t0
    print(f"[lockbox] Done in {elapsed:.1f}s. CCC={result.get('ccc_mean'):.4f}", flush=True)

    oof_path = out_dir / f"lockbox_self_norm_cross_{args.item}_{args.variant}_{timestamp}.oof.npy"
    ...
    json_path = out_dir / f"lockbox_self_norm_cross_{args.item}_{args.variant}_{timestamp}.json"
