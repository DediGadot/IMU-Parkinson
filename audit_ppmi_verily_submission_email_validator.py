#!/usr/bin/env python3
"""Audit the PPMI / Verily completed-email validator."""
...
SOURCE = ROOT / "scripts" / "ppmi_verily_submission_email_template.md"
VALIDATOR = ROOT / "scripts" / "validate_ppmi_verily_submission_email.py"
SYNTH_MD = RESULTS / "ppmi_verily_submission_email_validator_synthetic.md"
BAD_ANGLE_MD = RESULTS / "ppmi_verily_submission_email_validator_bad_angle.md"
BAD_SOURCE_MD = RESULTS / "ppmi_verily_submission_email_validator_missing_source.md"
OUT_JSON = RESULTS / "ppmi_verily_submission_email_validator_audit_20260515.json"
OUT_MD = RESULTS / "ppmi_verily_submission_email_validator_audit_20260515.md"
...
def run_validator(path: Path, allow_placeholders: bool = False) -> dict[str, Any]:
    cmd = ["uv", "run", "python", str(VALIDATOR), "--email", str(path)]
    ...
    try:
        parsed: dict[str, Any] | None = json.loads(proc.stdout)
...
def synthetic_completed_text() -> str:
    ...
    replacements = {
        ...
        "COMPLETED_PACKET_FILENAME": "synthetic_ppmi_verily_packet",
...
def main() -> None:
    ...
    bad_angle_result = run_validator(BAD_ANGLE_MD)
    ...
    bad_angle_parsed = bad_angle_result.get("parsed") or {}
    ...
    checks = [
        ...
        check(
            ...
            and template_parsed.get("checks", {}).get("placeholders_replaced", {}).get(
        ...
        check(
            "synthetic completed email passes without recording content",
            synthetic_result["returncode"] == 0
            and synthetic_parsed.get("passed") is True
            and synthetic_parsed.get("decision") == "completed_email_preflight_passed"
            ...
            and synthetic_parsed.get("email_identity_redacted") is True
            and synthetic_parsed.get("email_path_reported") is False
            and "email_path" not in synthetic_parsed
            ...
            and synthetic_parsed.get("goal_complete") is False,
            {
                ...
                "email_identity_redacted": synthetic_parsed.get("email_identity_redacted"),
                "email_path_reported": synthetic_parsed.get("email_path_reported"),
            },
        ),
        check(
        ...
        check(
            "validator permits only aligned recorder placeholders",
            synthetic_parsed.get("checks", {})
            .get("only_allowed_recorder_placeholders_remain", {})
            .get("passed")
            is True
            and bad_angle_result["returncode"] != 0
            and bad_angle_parsed.get("checks", {})
            .get("only_allowed_recorder_placeholders_remain", {})
            .get("passed")
            is False
            and "<unexpected_protected_detail>" in bad_angle_parsed.get("checks", {})
            .get("only_allowed_recorder_placeholders_remain", {})
            .get("unexpected_angle_placeholders", []),
            {
                "synthetic_allowed_remaining": synthetic_parsed.get("checks", {})
                .get("only_allowed_recorder_placeholders_remain", {})
                .get("remaining_angle_placeholders"),
                "bad_angle_returncode": bad_angle_result["returncode"],
                "bad_angle_unexpected": bad_angle_parsed.get("checks", {})
                .get("only_allowed_recorder_placeholders_remain", {})
                .get("unexpected_angle_placeholders"),
            },
        ),
        check(
            "validator output does not echo completed email path or filename",
            str(SYNTH_MD) not in synthetic_result["output_tail"]
            and SYNTH_MD.name not in synthetic_result["output_tail"]
            and str(BAD_ANGLE_MD) not in bad_angle_result["output_tail"]
            and BAD_ANGLE_MD.name not in bad_angle_result["output_tail"]
            ...
            and SOURCE.name not in template_result["output_tail"],
            {
                ...
                "bad_angle_output_contains_full_path": str(BAD_ANGLE_MD) in bad_angle_result["output_tail"],
                "bad_angle_output_contains_filename": BAD_ANGLE_MD.name in bad_angle_result["output_tail"],
        ...
        check(
            ...
            and template_allowed_parsed.get("decision") == "placeholder_tolerant_email_audit_passed"
            ...
            and template_allowed_parsed.get("checks", {}).get("placeholders_replaced", {}).get(
                "remaining_placeholder_count", 0
            )
            >= 10,
            {
    ...
    report = {
        ...
        "validator": rel(VALIDATOR),
        "synthetic_email": rel(SYNTH_MD),
        "passed": not hard_failures,
        "decision": "ppmi_verily_submission_email_validator_ready"
        if not hard_failures
        else "ppmi_verily_submission_email_validator_failed",
        "checks": checks,
