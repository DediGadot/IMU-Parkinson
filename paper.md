...
| Ours T1 (iter34 hybrid, hygiene-corrected candidate) | 92 PD | 13 body IMUs + 4 clinical | LOOCV | 1.736 | CCC = 0.7170 | Multi-task chain × 3-base-learner ensemble (NLS036 excluded for invalid item-15) |
...
| **iter34 hybrid (hygiene-corrected, strongest candidate)** | **8-item auxiliary chain × 3-base-learner ensemble {LGB, XGB-hist, ExtraTrees}; mean of chain outputs per fold per seed; NLS036 excluded** | **92** | **0.7170** | **1.736** | **`df89b9bb...`** |
| iter34 hybrid (historical N=93, retracted) | Same architecture, pre-validation loader (NLS036 included with invalid item-15=18) | 93 | 0.7366 | 1.731 | `df89b9bb...` |

iter12 honest CCC = 0.6550 on N=94 stands as the canonical T1 floor; the hygiene-corrected iter34 hybrid CCC = 0.7170 on N=92 is reported as the strongest candidate but does not formally replace it, because (i) the cohort drops to N=92 (NLS036 excluded for invalid auxiliary item-15 total; WPD002 excluded for missing item-18), (ii) the original N=93 lockbox CCC = 0.7366 was inflated by 0.020 by NLS036's invalid label acting as an unintended severity proxy and is hereby retracted, and (iii) family-wise multiple-comparisons accounting against the iter33-class probe family and the 2026-05-10/11 slot-D distillation/heterogeneous-base probe families is conservative (Section 4.12). A per-base disaggregation of the hygiene-corrected chain shows the numerically-highest single-base candidate (XGB-only, CCC = 0.7242 on N=92) does not clear the paired-bootstrap 0.95 gate vs the 3-base hybrid (frac > 0 = 0.728), confirming the averaging is well-calibrated for variance reduction at this N. Detailed comparisons and the iter33-A/B/C gate-rejection trio are documented in the supplementary material (`paper_supplement_iter33_gate_demo.md`).
...
The iter34 hybrid pipeline (Section 3.8) was originally locked at CCC = 0.7366 (MAE = 1.731, Pearson r = 0.7406, calibration slope = 0.8215) on N = 93 PD subjects, but this number was inflated by 0.020 by a single subject's invalid auxiliary item-15 label acting as an unintended severity proxy in chain training (see "Hygiene correction" below). The hygiene-corrected lockbox at N = 92 (excluding NLS036) gives CCC = 0.7170 (MAE = 1.736, Pearson r = 0.7223, calibration slope = 0.8151) on the T1 axial-plus-truncal subscore (items 9-14, range 0-24); per-seed CCC values are 0.7165, 0.7169, and 0.7175 (std = 0.0005). The hygiene-corrected number is the reproducible candidate; the original N = 93 number is retained below only as a historical-audit data point and is no longer cited as a deployable result.
...
**Hygiene correction (2026-05-10/11).** A second, later T1 label audit found an auxiliary-label defect with material impact at the LOOCV scale. `results/t1_iter48_aux_validrange_audit.json` shows that `NLS036` entered the historical iter34 auxiliary chain with item 15 = 18, above the valid item-15 total maximum of 8, because the raw item-15 right/left fields were coded 9/9. The reported T1 target items 9-14 for NLS036 are valid, so the headline target is not directly contaminated; however, a valid-range auxiliary filter reduces the chain cohort to N = 92. A follow-up audit (`results/t1_iter34_aux_order_audit.json`) falsified an initial fixed-order reassurance: because the chain order is random by seed, item 15 is upstream of T1 items in two of the three locked seeds, allowing the invalid label to act as a training feature for items 10, 12, and 13. The user-authorized hygiene-corrected lockbox (`results/preregistration_t1_iter34_hygiene_corrected_20260510_200037.json` → `results/lockbox_t1_iter34_hybrid_20260510_233019.json`) re-ran iter34 at N = 92 with the post-2026-05-09 validated loader (`updrs_columns.valid_updrs_item_total` added in commit `09d2e19`), using identical formula SHA-256 (`df89b9bb...`) and identical seeds, and obtained CCC = 0.7170 (MAE = 1.736, cal_slope = 0.8151) on a 27-min wall-time LOOCV pass with three seeds tightly clustered at 0.7165 / 0.7169 / 0.7175. Paired-bootstrap (n = 5,000) at N = 92 against iter5-direct gives paired Δ̄ = +0.0973, 95% CI [+0.017, +0.199], frac > 0 = 0.9908. The 0.020 drop from the original N = 93 number (0.7366 → 0.7170) is best interpreted as the unintended-severity-proxy effect of NLS036's invalid item-15 = 18 in the chain training rather than as a sample-size artifact, by analogy with the T3 iter47 valid-range correction (CCC 0.5227 → 0.3784) in 2026-05. We retract the historical N = 93 CCC = 0.7366 as a deployable result and report the hygiene-corrected N = 92 CCC = 0.7170 as the strongest reproducible T1 candidate. The original N = 93 number is retained only as a historical-audit data point.

**Per-item × per-base disaggregation (2026-05-11, FWER-free supplementary).** Marginalizing the hygiene-corrected chain output into per-item and per-base predictions (script `compute_t1_iter34_per_base_disaggregation.py`) produces a publishable per-item CCC table and a 6 × 3 per-item × per-base matrix at N = 92. T1-sum candidates per single base: LGB = 0.6964, XGB = 0.7242, ET = 0.7080 (mean of 3 seeds); the 3-base hybrid average is 0.7170. XGB-only is the highest single-base point estimate (Δ = +0.007 vs hybrid) but paired-bootstrap frac > 0 vs hybrid = 0.728, well below the 0.95 lockbox gate or the FWER-adjusted Bonferroni n = 8 gate (0.99375), confirming that the averaging is well-calibrated for variance reduction at this N. Per-item CCC at the hygiene-corrected hybrid ranges from item 13 = 0.067 (posture; IMU-noise) to item 12 = 0.565 (postural stability; clinical-augmented via the Stage-1 H&Y signal). Items 10 (gait, 0.443), 14 (body bradykinesia, 0.317), and 12 are the IMU-observable strengths; items 9 (chair rise, 0.234) and 11 (FoG, 0.232) are event-driven and partial; item 13 is geometry-bound and effectively a noise channel at this gait-IMU pipeline. The per-item × per-base matrix shows XGB winning items 10, 11, 12 (the ambulation-balance cluster), LGB winning items 9, 14 (the transient-transition cluster), and ET winning item 13 (the static-geometry channel, with CCC = 0.123 vs LGB 0.046 / XGB 0.034). This per-item base-preference profile motivates the 3-base ensemble but does not enable a closed-form heterogeneous-by-item-best chain at N = 92 without bilevel optimization that is not feasible at this sample size.
...
### 4.14 T1 Conformal Abstention: A Deployment-Mode Operating Point

The iter34 hygiene-corrected T1 hybrid achieves a LOOCV CCC of 0.7170 (MAE 1.736) on N=92 (Section 4.11). While additional in-cohort architectural improvements failed to clear the +0.025 MCID gate at this sample size, a clinically-deployable secondary operating mode emerges from conformal abstention via inter-model disagreement.

Two leakage-clean LOOCV predictors with overlapping cohorts are combined: the iter34 hybrid V2 prediction (CCC = 0.7170) and the V3-GSP single-family prediction (CCC = 0.7249). Each subject `i` has two independent predictions `p_V2(i)` and `p_GSP(i)`. The per-subject disagreement score is `d(i) = |p_V2(i) - p_GSP(i)|`. Split-conformal threshold selection uses a leave-one-subject-out quantile design: for each test subject `i`, the disagreement threshold at coverage `τ` is the `τ`-th percentile of the OTHER 91 subjects' disagreement scores; subject `i` is retained iff `d(i) ≤ threshold_{i,τ}`. The protocol is fully fold-local: no test-subject information enters the threshold calculation.

Pre-registered coverage targets are {100, 95, 90, 85, 80, 75, 70, 60, 50}%. Kill criteria were monotonicity violation (retained CCC at 50% < retained CCC at 70%) and threshold coefficient-of-variation > 0.20 across LOO calibrations.

Retained-subset metrics (V2-only predictor on retained subset):

| Coverage | Retained N | CCC | MAE | 95% Bootstrap CI | Threshold CV |
|---|---|---|---|---|---|
| 100% | 91 | 0.7140 | 1.75 | [0.568, 0.802] | 0.038 |
| 95% | 86 | 0.7413 | 1.69 | [0.578, 0.814] | 0.013 |
| 90% | 82 | 0.7465 | 1.69 | [0.593, 0.827] | 0.026 |
| 85% | 77 | 0.7514 | 1.68 | [0.588, 0.833] | 0.011 |
| 80% | 73 | 0.7511 | 1.71 | [0.586, 0.839] | 0.003 |
| 75% | 69 | 0.7612 | 1.69 | [0.593, 0.845] | 0.014 |
| **70%** | **64** | **0.7777** | **1.63** | **[0.604, 0.860]** | 0.024 |
| 60% | 55 | 0.8135 | 1.55 | [0.656, 0.881] | 0.007 |
| **50%** | **46** | **0.8338** | **1.33** | **[0.648, 0.908]** | 0.003 |

All threshold coefficients of variation across LOO calibrations are below 0.04 (kill threshold 0.20), the CCC curve is monotonic across all nine coverage levels, and the disagreement-error correlation is r(|p_V2-p_GSP|, |y - p_V2|) = 0.120 — weakly positive and statistically real at N=92. The 50/50 V2+V3-GSP blend predictor produces qualitatively identical results (70% coverage CCC = 0.7780, 50% CCC = 0.8332), confirming the abstention mechanism is robust to the choice of point predictor.

The retained-subset CCC at coverage `τ` is a **different estimand** from the standard LOOCV CCC reported in Section 4.11. It is the predictive concordance over the high-confidence subset of subjects — a clinically-relevant deployment-mode metric ("for the τ-fraction of cases where our model is most confident, how good is it?"). It does not supersede the iter34 LOOCV CCC of 0.7170 as a ceiling claim; rather, it provides a complementary operating point. A wearable-IMU-based T1 axial subscore estimator deployed under a disagreement-based abstention rule refers the most uncertain 30% of cases for in-person clinical assessment and produces a retained-subset CCC of 0.78 (MAE 1.63 on the 0-24 T1 axial subscore range, 95% CI [0.60, 0.86]) on the remaining 70%. At 50% retention, retained CCC reaches 0.83 (MAE 1.33). This is sufficient to support clinical-decision-support deployment for low-uncertainty cases while explicitly identifying high-uncertainty cases that warrant human review.

This is the only lift over iter34 LOOCV CCC=0.7170 that survived a 4-CLI (codex + kimi + deepseek + gemini) consensus design review during the post-codex-closure ceiling-push campaign of 2026-05-12. Eight in-cohort augmentation wildcards were tested (per-task Ridge specialist + Ridge meta, V3-GSP injection into T3 Stage-2, T3 conformal abstention with two predictor pairs, multi-block K=500 unified augmentation, stratified per-block K=500 quotas, V2+PSI isolation, stride-locked Stage-2 augmentation, and RandomForest base learner) and all failed to clear the +0.025 MCID gate under Bonferroni-adjusted inference at N=92/95. The T3 stride-locked 30-seed result produced a directional positive signal (mean Δ=+0.020, 25 of 30 seeds positive, binomial p ≈ 10⁻⁵) but a paired-bootstrap CI [-0.034, +0.067] crossing zero — the lift magnitude is sub-MCID at N=95 and reportable as a candidate for external replication, not as a ceiling break. Combining the stride lift with a K-selector substitution (univariate-corr K-best vs canonical LGB-importance K-best) gave a numerical lift of +0.044 over iter47 canonical but failed the corrected paired-bootstrap gate (frac>0 = 0.83, sign-flip p = 0.156).

The pre-registration JSON is `results/preregistration_goalv2_t1_conformal_lockbox_20260512.json` (status: locked, formula_sha256 captured at lock time); the lockbox JSON is `results/lockbox_t1_conformal_20260512_211440.json`; the producer script is `run_t1_conformal_lockbox.py`. Pre-registered inputs are `results/lockbox_t1_iter34_hybrid_20260510_233019.oof.npy` (V2 predictor) and `results/lockbox_t1_v3_gsp_v3_only_20260512_195152.oof.npy` (V3-GSP predictor); both pre-existing leakage-clean LOOCV OOF arrays.

## 5. Discussion

### 5.1 Principal Findings

**Framing note (post-audit reframing).** This paper was originally constructed around a single total-UPDRS-III regression headline of MAE = 6.89 (r = 0.860) from an LGB+XGB stacking ensemble (Sections 4.2, 5.2, 5.7). A subsequent leakage audit (2026-04-28; finding F65) found that the original SSL-ranking and stacking pipelines that produced those numbers contained transductive components: most consequentially a pre-computed XGBRanker fit on all 178 subjects whose leaf indices encoded test-fold rank, and a temperature scaling hyperparameter tuned on the same N = 94 LOOCV vector it was then evaluated on (cost: delta CCC about 0.343 on T1 5-fold; calibration slope pinned to 1.000 by construction). Later target-construction audits (2026-05-08; iter41 and iter47) found that the historical total-UPDRS-III target converted three all-missing Part III item blocks into zero labels and converted NLS036 item-15 missing codes into 18 severity points. After enforcing strict inductive evaluation, target hygiene, fold-local imputers and normalisers, single pre-registered LOOCV per pipeline, paired-bootstrap inference, and a five-null gate for new pipelines, the current audit-truth numbers are T1 LOOCV CCC = 0.6550 (MAE = 1.561; iter12 honest, N = 94), valid-range-corrected T3 LOOCV CCC = 0.3784 (MAE = 7.528; iter47 invalid-code fix, N = 95), and the hygiene-corrected T1 strongest candidate LOOCV CCC = 0.7170 (MAE = 1.736; iter34 hybrid at N = 92, NLS036 excluded for invalid auxiliary item-15 = 18). The historical N = 93 iter34 CCC = 0.7366 is retracted as a deployable result and retained only as a historical-audit data point. We retain the MAE = 6.89 / 6.43 figures and the old iter5 T3 CCC = 0.5227 throughout this Discussion as historical audit context only; the deployment-relevant numbers are the post-audit CCC values reported in Table 1, Table 1b, Section 4.11, Section 4.13, and Section 6.
...
### 6.1 Reproducibility checkpoint (canonical strings)
...
- T1 floor (canonical): T1 LOOCV CCC = 0.6550 (MAE = 1.561; iter12-honest, N = 94).
- T1 corrected candidate (post-publication replication target): T1 LOOCV CCC = 0.7170 (MAE = 1.736; hygiene-corrected iter34 hybrid, N = 92). The superseded original N=93 iter34 value was 0.7366 and is retained only as historical caveat evidence.
- T1 transportability (between-site): historical T1 LOSO transportability mean CCC = 0.456 (superseded original iter34 architecture, two-way mean across NLS → WPD and WPD → NLS).
- T3 canonical (valid-range corrected): T3 LOOCV CCC = 0.3784 (MAE = 7.528; iter47, N = 95).
- T3 transportability (between-site): T3 LOSO transportability mean CCC = 0.150 (iter47 valid-range, two-way mean).

For oracle / non-deployable bounds on T3 (gait-IMU only):

- theoretical oracle T1 plus mean R bound CCC = 0.351 (Bound A; oracle T1 plus mean residual on items 7–8 + 15–18).
- theoretical perfect T1 to T3 bound CCC = 0.683 (Bound D; perfect T1 propagated to T3).
- theoretical inductive shrinkage T1 pred to T3 bound CCC = 0.171 (Bound E; inductive-shrinkage T1_pred → T3, the achievable upper bound).

For the iter5 architecture, the Pareto-asymptote on the iter22 learning-curve sweep is `CCC(N) = 0.5975 − 2.1308·N^(−0.6408)` (Pareto AIC = −52.75 vs loglinear AIC = −39.22), giving a structural T3 ceiling of 0.5975 that more cohort N alone cannot break.
...
### Current post-audit figures (Nature-grade, derived from `claim_ledger.json`)

The ten figures below are produced by `scripts/paper_figures.py` from the typed claim ledger at `results/current_paper_export/claim_ledger.json`. Every numeric annotation traces to a ledger `claim_id`; the script is enforced no-hard-code via AST scan.

![Figure 1: Audit timeline.](figures/current/fig01_audit_timeline.png)
...
![Figure 2: Observability ceiling.](figures/current/fig02_observability_ceiling.png)

**Figure 2.** The gait-observable T1 axial subscore (items 9–14) is learnable at CCC ≈ 0.66 floor / 0.72 hygiene-corrected candidate; total UPDRS-III (T3) collapses to CCC ≈ 0.38 internal and 0.15 between-site, well below the theoretical Bound A oracle (0.351) and Bound D oracle perfect-T1→T3 (0.683). The Pareto N→∞ asymptote (0.5975) for the iter5 architecture is plotted as a structural ceiling that more data alone cannot break. Frame T3 as a strict-inductive cautionary benchmark, not a deployable result.

![Figure 3: iter34 vs iter12 paired bootstrap.](figures/current/fig03_iter34_vs_iter12_paired.png)

**Figure 3.** Subject-level paired bootstrap on the historical n = 93 common cohort. The original iter34 run (CCC = 0.7366) lifted iter12-honest (CCC = 0.6550), but it is now superseded for current-candidate citation by the N=92 hygiene-corrected rerun (CCC = 0.7170). iter34 is therefore reported as a **hygiene-corrected candidate / post-publication replication target**, not a canonical replacement.

![Figure 4: Transportability cliff.](figures/current/fig04_transportability_cliff.png)

**Figure 4.** Internal LOOCV systematically over-estimates between-site transportability. The superseded original T1 iter34 run dropped from 0.7366 to 0.4564 (Δ = −0.280), while the current hygiene-corrected internal candidate is 0.7170; T3 iter47 drops from 0.3784 to 0.150 (Δ = −0.228). Both deltas argue against treating LOOCV CCC as a deployment estimate.

![Figure 5: T3 target hygiene waterfall.](figures/current/fig05_t3_target_hygiene_waterfall.png)

**Figure 5.** The T3 headline changed because labels were corrected, not because modelling got worse. The historical iter5 CCC = 0.5227 and iter16 IPW CCC = 0.4694 used a target built by skipna-summing 33 raw Part III subitems, which converted three all-missing rows into true zero labels and treated `NLS036` 9/9 invalid codes as 18 severity points. iter41 excluded all-missing rows (CCC = 0.3948); iter47 additionally recoded out-of-range raw codes as missing (CCC = 0.3784). LOSO transportability falls further to 0.150.

![Figure 6: Item-level observability map.](figures/current/fig06_item_level_observability_map.png)
...
![Figure 7: Leakage audit gate matrix.](figures/current/fig07_leakage_audit_gate_matrix.png)
...
![Figure 8: Learning curve asymptote.](figures/current/fig08_learning_curve_asymptote.png)

**Figure 8.** Pareto fit to the iter22 learning-curve sweep yields `CCC(N) = 0.5975 − 2.1308·N^(−0.6408)` (Pareto AIC = −52.75 vs loglinear AIC = −39.22). The asymptote 0.5975 sets a structural ceiling for the iter5 architecture that more cohort N alone cannot break. Reaching deployment-grade T3 CCC therefore requires an architecture or feature change, not just data scaling.

![Figure 9: T3 residual anatomy.](figures/current/fig09_t3_residual_anatomy.png)
...
![Figure 10: External transportability context.](figures/current/fig10_external_transportability_context.png)
...
### Historical / legacy figure list (retained for archival completeness)
