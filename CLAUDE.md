...
**T3 iter25 (2026-05-05) — Cross-dataset zero-shot transportability on PADS — NO TRANSFER (F60).** First published cross-dataset zero-shot transportability number for the WearGait-PD-trained iter5 architecture. Target: PADS (PhysioNet Parkinson's Disease Smartwatch dataset; German cohort; Apple Watch wrist; 79 HC + 276 PD). 310 subjects extracted (243 PD + 67 HC), 69 common wrist features (3-axis acc + magnitude → time/freq/gait_reg). Pre-registered single-batch (`results/preregistration_t3_iter25_pads_20260505_073324.json`, formula_sha256 `9972a6d163382174`); 3 tracks evaluated in one batch. **Track A** (V2-wrist LGB, no clinical Stage 1): AUROC = **0.5166** (chance). **Track B** (iter5 Stage 1+2 with mean-imputed PADS clinical): AUROC = **0.4177** (worse-than-chance — mean-imputed Stage 1 collapses to constant; Stage-2 LGB extrapolating on OOD wrist features gives inverted predictions per codex+gemini synthesis). **Track C** (PADS-only 5-fold baseline): AUROC = **0.6336 ± 0.019** (upper bound). 0.11-AUROC gap (Track C vs A) is expected for cross-device wrist transfer (Movella → Apple Watch + country shift + protocol shift). **Verdict: NO TRANSFER (AUROC < 0.55 threshold).** Paper Table 3 transportability gradient now reads as a **cascading transportability cliff**: iter5 LOOCV CCC=0.5227 (internal) → iter16 LOSO CCC=0.341 (intra-cohort site shift within WG) → iter25 PADS AUROC=0.5166 (external zero-shot collapse). Strongest negative finding of the mission; **internal validation drastically overestimates real-world clinical readiness** — the headline message of the cautionary-benchmark paper. Findings F60.
