...
## PPMI replication data (2026-05-21 update; inventory verified live 2026-05-23 — labels + derived gait features unlocked; raw IMU + raw imaging still gated)

Location: `ssh -p 2243 fiod@165.22.71.91:/home/fiod/PPMI/` — **309 CSVs + 6 methods PDFs (315 files total), 4.8 GB, single flat directory; extensions are exclusively csv + pdf (verified live 2026-05-23) — zero image/raw-sensor files**. Filename suffix `_20May2026.csv` (193 CSVs) or `_21May2026.csv` (115 CSVs, including labels + Stage-1 covariates + derived gait features); some files are duplicated across the two pulls (de-dup by latest suffix).
...
- **Raw imaging (DICOM/NIfTI) is also absent (verified live 2026-05-23).** The PPMI dump is **csv + pdf only**; the Imaging (DAT-SPECT / MRI / DTI / PET) and -omics entries listed above are **derived tabular scalars / acquisition metadata, NOT image files**. A Qwen-VL *vision* phase over actual scans therefore needs a **separate LONI IDA imaging request** — the same gate as raw IMU. The multimodal that is executable now from this dump is *tabular* fusion, not vision.
