...
def main(null_mode: str = ""):
    ...
    out = {
        "name": "lockbox_t1_slotD_conformal_ph_correction",
    ...
    path = Path(f"results/lockbox_t1_slotD_conformal_ph_correction_{ts}{suffix}.json")
...
def sanity_y_nan():
    ...
    receipt = {
        "name": "abstention_sanity_t1_slotD_conformal_ph_correction",
        "created_at_utc": datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ"),
