...
This repository is organized around standalone experiment scripts rather than a packaged `src/` tree. Core shared logic lives in `data_split.py`; keep it as the single source of truth for dataset paths, subject splits, and windowing. The current clean-path runners are `run_clean_benchmark.py`, `run_stats_report.py`, `run_sensor_ablation.py`, `run_loocv_stack.py`, `run_dl_experiments.py`, and `run_dl_rebenchmark.py`. Rebuild the manuscript with `generate_html_paper3.py`. Generated plots belong in `figures/`, pulled logs and metrics in `results/`, and narrative notes in Markdown files such as `findings.md`.
...
- `python run_clean_benchmark.py` or `python run_stats_report.py`: rerun a corrected analysis and refresh JSON outputs in `results/`.
- `python generate_html_paper3.py`: rebuild the corrected HTML manuscript.
...
## Testing Guidelines
There is no formal `tests/` suite yet. Validate changes by rerunning the affected script and checking the expected artifact: JSON in `results/`, plots in `figures/`, or the rendered `paper3.html`. For data-split or evaluation changes, confirm subject-level splitting is preserved and report before/after metrics such as MAE and correlation.
