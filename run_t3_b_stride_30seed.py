"""T3-B stride 30-seed LOOCV — final seed-mean stability test.

10-seed at 2026-05-12T21:36Z showed mean Δ=+0.023, std=0.026, 7/10 positive,
4/10 ≥+0.044. The high right-skew variance may stabilize at more seeds.

Decision rule (pre-registered):
  - If 30-seed mean Δ ≥ +0.025 AND std < 0.015 AND frac>0 ≥ 0.95: PASS_CANDIDATE.
  - If mean ≥ +0.015 AND std < 0.020: WEAK_POSITIVE_NEEDS_EXTERNAL.
  - Else: FAIL_NOISE_DOMINATED_FINAL.

This is the final wildcard of goal-v2. After this the campaign is closed.
"""
...
K_BEST = 500
SEEDS = tuple(range(1, 31))  # 30 seeds: 1, 2, ..., 30
...
def main():
    print("=" * 72)
    print("T3-B stride 30-seed LOOCV — FINAL stability test")
    print("=" * 72)
    print(f"  Seeds: {SEEDS}")
    ...
    for seed_idx, seed in enumerate(SEEDS):
        ...
        print(f"  seed {seed_idx+1}/30 = {seed}: base={ccc(y, p_base):.4f} aug={ccc(y, p_aug):.4f} "
              f"Δ={delta:+.4f} elapsed={elapsed:.1f}s", flush=True)
    ...
    summary = {
        ...
        "n_seeds_above_mcid": n_above_mcid,
        "paired_bootstrap": {
            "n_boot": n_boot,
    ...
    out_path = REPO_ROOT / "results" / f"lockbox_t3_b_stride_30seed_{ts}.json"
    out_path.write_text(json.dumps(summary, indent=2))

    print(f"\n  === FINAL (30 seeds) ===")
    print(f"  mean Δ = {delta_mean:+.4f}, std = {delta_std:.4f}, median = {delta_median:+.4f}")
    print(f"  pooled Δ = {pooled_delta:+.4f}")
    print(f"  positive seeds: {n_positive}/30, above-MCID: {n_above_mcid}/30")
    print(f"  frac>0 = {frac_gt:.4f}, CI [{ci_lo:+.4f}, {ci_hi:+.4f}]")
