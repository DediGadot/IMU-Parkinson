"""T3 V2 + PSI + stride combined — narrower 3-block combo than comprehensive_aug.

comprehensive_aug (V2+GSP+PSI+stride+shapelet) HURT by Δ=-0.053.
V2+PSI alone HURT by Δ=-0.062.
V2+stride alone gave +0.020 mean (30-seed).
Test: V2 + PSI + stride only — maybe PSI+stride synergy that pure additions miss.

Architecture: K=500 univariate-corr on V2+PSI+stride pool, iter47-arch LOOCV, 3 seeds.
"""
...
def main():
    ...
    X_psi, _ = load_csv(REPO_ROOT / "results" / "v3_psi_features.csv", sids)
    X_stride, _ = load_csv(REPO_ROOT / "results" / "stride_locked_subj.csv", sids)
    X_aug = np.column_stack([X_v2, X_psi, X_stride])
    print(f"  V2={X_v2.shape[1]}, PSI={X_psi.shape[1]}, stride={X_stride.shape[1]}, aug={X_aug.shape[1]}")
    ...
    for seed in SEEDS:
        ...
        print(f"  seed={seed} base={ccc(y, p_base):.4f} aug={ccc(y, p_aug):.4f} Δ={delta:+.4f}", flush=True)
    ...
    summary = {
        ...
        "verdict": "PASS" if delta_pooled >= 0.025 else "MARGINAL" if delta_pooled >= 0.010 else "FAIL",
    }
    ts = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    out = REPO_ROOT / "results" / f"lockbox_t3_psi_plus_stride_{ts}.json"
    out.write_text(json.dumps(summary, indent=2))
    print(f"\n  Δ_pooled = {delta_pooled:+.4f}")
